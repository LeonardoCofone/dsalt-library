...
def _hybrid_scores(
    ...
    return (alpha.unsqueeze(0) * z_v + (1.0 - alpha.unsqueeze(0)) * z_x.unsqueeze(1)).mean(dim=1)
...
def _build_mask_batched(
    ...
    k_act       = min(k_lmk, int((scores > float("-inf")).sum()))
    if k_act > 0:
...
class DSALTAttention(nn.Module):
    ...
    def _rope(self, n: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return build_rope_cache(n, self.head_dim, device, scale=self.yarn_scale)
    ...
    def _batched(self, x: torch.Tensor, B: int, T: int, device: torch.device) -> torch.Tensor:
        ...
        if not self.training:
            ...
            self._last_attn = torch.softmax(sc + additive, dim=-1).detach()
        else:
    ...
    def _packed(self, x: torch.Tensor, cu_seqlens: torch.Tensor, total_len: int, device: torch.device) -> torch.Tensor:
        ...
        if not self.training:
            ...
            with torch.no_grad():
                ...
                self._last_attn = torch.softmax(sc + ad, dim=-1).detach()
        else:
            self._last_attn = None
